...
class Game():
    def __init__(self):
        ...
        self.player = Player(SCR_W/2, SCR_H/2)
    ...
    def drawTunnel(self):
        ...
        for wall in sorted(self.walls,key=lambda x: -x.pos[2]):
            wall.render(self.output,-self.player.xpos+SCR_W/2,-self.player.ypos+SCR_H/2)
    ...
    def drawPlayer(self):
        ...
        if FIRST_PERSON:
            self.output.blit(sprite, (SCR_W/2-sprite.get_width()/2, SCR_H/2-sprite.get_height()/2))
            return
        self.output.blit(sprite, (self.player.xpos-sprite.get_width()/2, self.player.ypos-sprite.get_height()/2))
